...
# -*- coding: utf-8 -*-
"""
circle_clusters_v1f.py — circles → clusters with enriched metrics; always shows plot at end (PyCharm-friendly).
- No CLI flags for show/save. It just runs, writes CSVs, and then shows the plot window.
- Uses a default IGC path if you press Enter at the prompt.
Outputs:
...
# --- main ---
def main():
    ap = argparse.ArgumentParser(description="Circles → clusters with enriched metrics; always shows plot at end")
    ...
    logger.info(f"[clusters v1f] Fixes: {len(df)}")
    ...
    # Always show the plot at the very end
