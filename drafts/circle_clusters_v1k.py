...
# -*- coding: utf-8 -*-
"""
circle_clusters_v1k.py — edge-wise distributed labels (ticks) using full edge space; CSVs written to debug dir.
- Assign clusters to nearest polygon edge.
- For each edge, distribute label positions evenly along the edge length (use the room).
- Labels pushed outward from the polygon.
- Markers: black X; Labels: "id, n_segments".
- CSVs go to /Users/denisbuckley/PycharmProjects/chatgpt_igc/debugs/ and paths are printed.
"""
...
# --- edge-based label placement (use the room) ---
BASE_OFFSET_M = 100.0      # push labels outward
MIN_DS_M      = 120.0      # minimum spacing along an edge between labels
MARGIN_FRAC   = 0.08       # keep labels away from edge endpoints (8% on each side)

def edge_length_m(lat1, lon1, lat2, lon2):
...
def plot_overlay(df, clusters):
    ...
    # place labels per edge using full edge length
    for edge_i, items in by_edge.items():
        # sort by projection for stable ordering
...
# --- main ---
def main():
    ap = argparse.ArgumentParser(description="Circles → clusters with edge-wise distributed labels (outside)")
    ap.add_argument("igc", nargs="?", help="Path to IGC file")
    # Defaults write to DEBUG_DIR, but allow override
    ap.add_argument("--clusters-csv", default=os.path.join(DEBUG_DIR, "circle_clusters_enriched.csv"))
    ap.add_argument("--segments-csv", default=os.path.join(DEBUG_DIR, "../parking/circle_segments.csv"))
    args = ap.parse_args()
