...
# =========================
# Circles-only detector (strict ~360°, time-based cap) + plotting + DEBUG
# =========================
...
def find_circles_360_timecap_debug(df: pd.DataFrame,
                                   min_circle_heading_deg: float = 340.0,
                                   max_circle_heading_deg: float = 380.0,
                                   tiny_step_deg: float = 0.2,
                                   max_circle_duration_s: float = 600.0):
    """
    Return (circles, attempts) where:
      circles  = [(start_idx, end_idx, cum_deg)]
      attempts = list of dicts with keys:
                 start, end, samples, dur_s, direction, cum_deg, outcome
    """
    df = _ensure_sec(df)
    n = len(df)
    if n < 3:
    ...
    while i < n - 1:
        ...
        while j < n and (j - start) <= max_len_samples:
            ...
            if abs(step) < tiny_step_deg:
                j += 1
                continue
            # direction reversal ends this attempt
            if (step > 0 and direction == 'left') or (step < 0 and direction == 'right'):
                outcome = 'reversed'
                break
            cum += step
            ac = abs(cum)
            if min_circle_heading_deg <= ac <= max_circle_heading_deg:
                ...
                outcome = 'closed'
                i = j  # jump to end of this circle
                break
            j += 1
        else:
            # ran out or exceeded time cap
            if (j - start) > max_len_samples:
                outcome = 'timeout'
            else:
                outcome = 'end_of_trace'
            i = j

        # Log attempt
        end = min(j, n-1)
        dur_s = float(secs[end] - secs[start]) if end > start else 0.0
        attempts.append({
            'start': start, 'end': end, 'samples': (end - start),
            'dur_s': dur_s, 'direction': direction, 'cum_deg': cum, 'outcome': outcome
        })

        if outcome == 'closed':
            # continue after closed circle (i already set)
            continue
        # otherwise i already set to j by the branch above
...
def plot_circles(df: pd.DataFrame, circles):
    ...
    ax.set_title("Circles-only detection (360°, time-capped) — DEBUG")
...
def main():
    ...
    print(f"[circles-360-timecap-DEBUG] Parsing IGC: {path}")
    df = parse_igc(path)
    ...
    if tow and isinstance(tow, tuple) and len(tow) == 2:
        s, e = tow
        print(f"[circles-360-timecap-DEBUG] Tow detected: {s}→{e}. Excluding tow.")
        df = df.iloc[e+1:].reset_index(drop=True)
    else:
        print("[circles-360-timecap-DEBUG] Tow not clearly detected; using full trace.")

    # Detection
    circles, attempts = find_circles_360_timecap_debug(df)

    # Cadence and totals
    print(f"[circles-360-timecap-DEBUG] Cadence ≈ {_sec_per_fix(df):.2f} s/fix")
    print(f"[circles-360-timecap-DEBUG] Circles: {len(circles)}, Attempts: {len(attempts)}")

    # Print all attempts (can be long)
    for a in attempts:
        print(f"  attempt: idx {a['start']}→{a['end']}  samp {a['samples']:4d}  "
              f"dur {a['dur_s']:7.1f}s  dir {a['direction']:<5}  arc {a['cum_deg']:7.1f}°  outcome={a['outcome']}")
