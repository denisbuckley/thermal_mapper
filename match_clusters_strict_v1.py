...
#!/usr/bin/env python3
"""
match_clusters_strict_v1.py

Strict one-to-one matching between *circle* and *altitude* enriched clusters.
- Reads the latest enriched CSVs from ./outputs:
    - circle_clusters_enriched_*.csv
    - altitude_clusters_enriched_*.csv
- Builds candidate pairs subject to spatial and temporal constraints.
- Selects a STRICT 1:1 set of matches using a greedy best-first assignment.
- Writes outputs/matched_clusters_<ts>.csv and prints a console summary.

Scoring & Constraints (tweak at top of file):
- Spatial threshold: EPS_M (meters) maximum center separation.
- Temporal requirement: either overlap >= MIN_OVL_FRAC of the shorter window,
  or, if no overlap, the start/end gap must be <= MAX_TIME_GAP_S (seconds).
- Greedy score: (distance_km, -overlap_frac, gain_rate_penalty)
  → smallest distance first, prefer higher time overlap, then smaller gain/rate diffs.

Author: chatgpt — strict matcher v1
"""
...
def load_enriched():
    ...
    if not circ or not alti:
        print("[match_strict] Missing enriched inputs. Run enrichment first.")
    ...
    for df in (C, A):
        if 'cluster_id' not in df.columns and 'thermal_id' in df.columns:
            df = df.rename(columns={'thermal_id':'cluster_id'})
        # coerce times
...
def build_candidates(C, A):
    """Return list of candidate dicts with keys:
       circle_id, alt_id, dist_m, overlap_s, overlap_frac, time_gap_s, gain_diff_abs, rate_diff_abs, score
    """
    crows = C.to_dict('records'); arows = A.to_dict('records')
    cands = []
    for cr in crows:
        for ar in arows:
            ...
            # compute min duration for overlap fraction
            ...
            # differences to help break ties
            ...
            # normalize penalty (avoid NaNs)
            gain_pen = 0.0 if (math.isnan(gain_diff)) else min(1.0, gain_diff / 200.0)  # cap at ~200 m
            rate_pen = 0.0 if (math.isnan(rate_diff)) else min(1.0, rate_diff / 5.0)    # cap at 5 m/s
            # score tuple
            ...
            cands.append(dict(
                ...
                time_gap_s = gap_s,
                gain_diff_abs = None if math.isnan(gain_diff) else gain_diff,
                rate_diff_abs = None if math.isnan(rate_diff) else rate_diff,
                score = score
            ))
...
def main():
    ...
    print(f"[match_strict] Candidate pairs: {len(cands)}")

    pairs = greedy_assign(cands)
    print(f"[match_strict] Strict 1:1 matches: {len(pairs)}")
    ...
    for p in pairs:
        rows.append(dict(
            ...
            gain_diff_m= (None if p['gain_diff_abs'] is None else round(p['gain_diff_abs'],1)),
            rate_diff_mps=(None if p['rate_diff_abs'] is None else round(p['rate_diff_abs'],2))
        ))
    M = pd.DataFrame(rows)
    ...
    print(f"[match_strict] Wrote {out_csv}")
    ...
    if not M.empty:
        ...
        for _, r in M.iterrows():
            print(f"{int(r['circle_id']):9d}  {int(r['alt_id']):6d}  {r['dist_km']:7.3f}  {r['overlap_s']:9.1f}"
                  f"  {r['overlap_frac']:12.3f}  {r['time_gap_s']:10.1f}"
                  f"  {'' if pd.isna(r['gain_diff_m']) else f'{r['gain_diff_m']:.1f}':>11}"
                  f"  {'' if pd.isna(r['rate_diff_mps']) else f'{r['rate_diff_mps']:.2f}':>14}")
    else:
        print("[match_strict] No matches under current thresholds. Consider widening EPS_M or lowering MIN_OVL_FRAC.")
