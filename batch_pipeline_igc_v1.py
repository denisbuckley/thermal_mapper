...
End-to-end batch runner:
  For every *.igc in ./igc (by default), it will:
...
DEFAULTS
- igc_dir: ./igc   (next to this script)
...
USAGE
  ...
  python batch_pipeline_igc_v1.py --igc-dir data/igc --alt-script my_alt.py --circ-script my_circ.py --force
...
def run_detector_script(script_path: Path, igc_path: Path, log, label:str):
    ...
    Supports two modes:
      - If the script accepts CLI arg --igc, we pass it.
    ...
    try:
        r = subprocess.run(
            [sys.executable, str(script_path), "--igc", str(igc_path)],
