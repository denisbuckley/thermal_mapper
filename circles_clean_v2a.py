...
"""
circles_clean_v2a.py
====================
Same as v2 (no labels), but make green circle markers clearly visible:
- Draw cluster centers first (smaller, slight alpha)
- Draw green circle markers AFTER, larger linewidth & zorder so they are on top
"""
...
# Marker styling (visibility)
CENTER_S = 70          # size of black X centers
CENTER_LW = 1.4
CENTER_ALPHA = 0.85

CIRCLE_S = 80          # size of green X circles (on top)
CIRCLE_LW = 2.0
CIRCLE_Z = 7
...
def plot_map(df, circles, clusters_df, dt):
    ...
    # Cluster centers FIRST (smaller), so green circles drawn after remain visible
    big = clusters_df[clusters_df["n"] >= MIN_CLUSTER_COUNT]
    if not big.empty:
        ax.scatter(big["center_lon"], big["center_lat"], marker='x', c='k',
...
def main():
    ...
    print(f"[circles-clean v2a] Parsing IGC: {igc_file}")
    df = parse_igc(igc_file)
    if df.empty:
        print("[circles-clean v2a] No rows parsed; aborting."); return
    df = compute_derived(df); df = ensure_time_sec(df)

    try:
        tow = detect_tow_segment(df)
    except Exception as e:
        print(f"[circles-clean v2a] Tow detect error: {e}"); tow = None
    if isinstance(tow, tuple) and len(tow) == 2:
        s,e = tow
        print(f"[circles-clean v2a] Tow detected: {s}→{e}. Excluding tow.")
        df = df.iloc[e+1:].reset_index(drop=True); df = ensure_time_sec(df)
    else:
        print("[circles-clean v2a] Tow not clearly detected; using full trace.")
    print(f"[circles-clean v2a] Points after tow trim: {len(df)}")

    circles, dt = detect_circles(df, MIN_ARC_DEG, MAX_LEN_SAMPLES, HEADING_SMOOTH_S)
    print(f"[circles-clean v2a] Cadence ≈ {dt:.2f} s/fix | circles detected: {len(circles)}")
    ...
    print(f"[circles-clean v2a] Wrote CSVs to ./outputs: {f1.name}, {f2.name}, {f3.name}")
