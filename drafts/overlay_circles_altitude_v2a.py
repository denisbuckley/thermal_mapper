...
Author: chatgpt-igc overlay v1k
"""

# --------------------------- USER SETTINGS ---------------------------
DEFAULT_IGC = "2020-01-11 Norm Bloch 106248.igc"
...
try:
    ...
    warnings.warn(f"tuning_loader not available or failed: {e}")
...
def plot_overlay(df, circles, circle_clusters, alt_clusters, banner_text, out_csv_path):
    ...
    if not circle_clusters.empty:
        ...
        print(f"[overlay v1k] Wrote circle clusters CSV: {out_csv_path}")
    plt.show()

# -------------------- MAIN --------------------
def main():
    igc_path = DEFAULT_IGC
    print(f"[overlay v1k] Parsing IGC: {os.path.basename(igc_path)}")
    df = load_igc(igc_path)
    print(f"[overlay v1k] Fixes: {len(df)}")

    df2, tow = maybe_exclude_tow(df)
    if tow:
        print(f"[overlay v1k] Tow excluded: {tow[0]}→{tow[1]}")
    else:
        print("[overlay v1k] Tow exclusion: heuristic (first 8 min)")

    # Detect circles
    circles = detect_circles(df2)
    print(f"[overlay v1k] Circles detected: {len(circles)}")

    # Cluster circles
    circ_with_id, circ_clusters = cluster_circles(
        circles, eps_m=CL_EPS_M, gap_s=CL_GAP_S, min_count=CL_MIN_COUNT
    )
    print(f"[overlay v1k] Circle clusters kept (n>={CL_MIN_COUNT}): {len(circ_clusters)}")
    ...
    if alt_clusters is None or alt_clusters.empty:
        print("[overlay v1k] Altitude clusters: 0")
    else:
        print(f"[overlay v1k] Altitude clusters: {len(alt_clusters)}")
