...
# =========================
# Circles-only detector tuned for ~18–45 s circles + plot
# (Based on bank 30–40° and speed 50–70 kt)
# =========================

def _signed_delta_deg(a: float, b: float) -> float:
    """Signed shortest-angle delta a->b in (-180, 180]."""
    d = b - a
    if d > 180.0: d -= 360.0
...
def find_circles_360_tuned(df: pd.DataFrame,
    ...
    """
    Detect full circles (~360°) with:
      - consistent turn direction,
      - duration within [target_min_dur_s, target_max_dur_s] (converted via cadence),
      - average |turn rate| within bounds (deg/s).
    Returns list of (start_idx, end_idx, arc_deg, dur_s, mean_abs_tr_dps).
    """
    df = _ensure_sec(df)
    n = len(df)
    if n < 3:
    ...
    # Convert duration window to samples (+/- 1 sample slack)
    ...
    while i < n - 1:
        ...
        sum_abs = 0.0
        j = i + 1
        while j < n and (j - start) <= (max_samp + 2):  # small slack
            ...
            # Only evaluate once we are in the sample-duration window
            if s_count >= min_samp and s_count <= max_samp and min_arc_deg <= arc <= max_arc_deg:
                # average absolute turn rate (deg/s)
            ...
            # exceeded the window without closing
...
def main():
    ...
    print(f"[circles-360-tuned] Parsing IGC: {path}")
    df = parse_igc(path)
    ...
    if tow and isinstance(tow, tuple) and len(tow) == 2:
        s, e = tow
        print(f"[circles-360-tuned] Tow detected: {s}→{e}. Excluding tow.")
        df = df.iloc[e+1:].reset_index(drop=True)
    else:
        print("[circles-360-tuned] Tow not clearly detected; using full trace.")

    circles, dt_fix = find_circles_360_tuned(df)
    print(f"[circles-360-tuned] Cadence ≈ {dt_fix:.2f} s/fix")
    print(f"[circles-360-tuned] Detected {len(circles)} circles (target ≈18–45 s)")
    for (s, e, cum, dur_s, mean_tr) in circles:
    ...
    if len(circles) > 0:
        ...
        print("[circles-360-tuned] No circles to plot.")
