...
# -*- coding: utf-8 -*-
"""
circle_clusters_v1g.py — circles → clusters with enriched metrics; always shows plot at end (PyCharm-friendly).
Plotting:
  - Track line in light blue
  - Clusters marked as modest black X's
  - Labels: "cluster_id, n_segments" placed slightly offset
Outputs:
...
def plot_overlay(df, clusters):
    ...
    for _, r in clusters.iterrows():
        ...
        ax.text(r["lon"], r["lat"], f"{int(r['cluster_id'])}, {int(r['n_segments'])}",
...
# --- main ---
def main():
    ap = argparse.ArgumentParser(description="Circles → clusters with simple plot labels")
