    ...
    while i < n:
        ...
        if rot >= 360.0:
            ...
            for k in range(i0 + 1, i1 + 1):
                ...
                v_inst = seg_m / dt
                b_rad = math.atan((v_inst * omega_k) / g) if np.isfinite(v_inst) and np.isfinite(omega_k) else float(
                    "nan")
            ...
            if banks_deg:
                ...
                bank_std_deg = np.nan;
                bank_range_deg = np.nan;
                bank_masd_deg = np.nan
...
def load_track_for_plot(run_dir: Path, stem: str) -> pd.DataFrame:
    ...
    return pd.DataFrame(
        {"lat": df["lat"], "lon": df["lon"], "alt": df.get("alt", df.get("alt_gps", df.get("alt_pressure", np.nan))),
         "time": pd.NaT})
...
def render_one(stem: str, run_dir: Path, show: bool = True) -> None:
    ...
    plot_path_png = run_dir / f"plot_{stem}.png"
    plot_path_svg = run_dir / f"plot_{stem}.svg"

    fig.savefig(plot_path_png, dpi=150, bbox_inches="tight")
    fig.savefig(plot_path_svg, bbox_inches="tight")
    print(f"[OK] wrote plots → {plot_path_png}, {plot_path_svg}")
...
def main() -> int:
    ...
    print(
        f"[SUMMARY] circles={len(circles)}, circle_clusters={len(cc)}, altitude_clusters={len(alts)}, matched={len(matches)}, thermals={len(thermals)}")
