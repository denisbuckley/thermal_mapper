...
Single prompt for IGC path. No subprocess, no external imports from project scripts.
...
def cluster_circles(circles_df, dist_thresh=200.0, time_gap=300.0):
    ...
    for c in clusters:
        ...
        enriched.append({
            ...
            "n_segments": len(cdf),  # count of circles (keep header consistent with altitude CSV)
...
def detect_altitude_clusters(igc_path,
    ...
    # velocity via centered finite diff over window
    if len(t) < 3:
        return pd.DataFrame(columns=[
            "cluster_id","n_segments","n_turns_sum","duration_min","alt_gained_m",
            "av_climb_ms","lat","lon","t_start","t_end"
    ...
    # build clusters (here each segment is its own cluster)
    rows = []
