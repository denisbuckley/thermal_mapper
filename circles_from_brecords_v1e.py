...
def detect_circles(df,
    ...
    while i < n:
        ...
        if rot >= 360.0:
            ...
            # compute per-sample instantaneous bank via v*omega/g
            banks_deg = []
            for k in range(i0 + 1, i1 + 1):
                dt = t[k] - t[k - 1]
                if dt <= 0:
                    continue
                # angular velocity (rad/s) from unwrapped heading
                dtheta_rad = math.radians(uw[k] - uw[k - 1])
                omega = dtheta_rad / dt
                # ground speed (m/s) from segment distance
                seg_m = haversine_m(lat[k - 1], lon[k - 1], lat[k], lon[k])
                v = seg_m / dt
                # instantaneous bank angle (rad)
                b_rad = math.atan((v * omega) / g) if np.isfinite(v) and np.isfinite(omega) else float("nan")
            ...
            if banks_deg:
                ...
                if len(b_arr) >= 2:
                    masd = float(np.nanmean(np.abs(np.diff(b_arr))))
                else:
                    masd = 0.0
            else:
...
def main():
    ...
    # Ensure the IGC is present in the run_dir (symlink preferred; falls back to copy)
    ...
    canonical = [
        ...
        "circle_id", "seg_id", "avg_speed_kmh", "turn_radius_m", "circle_diameter_m", "bank_angle_deg",  "bank_std_deg", "bank_range_deg", "bank_masd_deg"
    ]
