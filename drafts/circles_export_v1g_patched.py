...
def plot_map_with_circles_and_clusters(df: pd.DataFrame, circles, clusters_df: pd.DataFrame, dt: float):
    ...
    # Circles as green crosses at midpoints
    mids_lon, mids_lat = [], []
    for (s,e,arc) in circles:
        mid = (s + e) // 2
        mids_lon.append(float(df['lon'].iloc[mid]))
        mids_lat.append(float(df['lat'].iloc[mid]))
    if len(mids_lon):
        ax.scatter(mids_lon, mids_lat, marker='x', c='green', s=36, linewidths=1.5, zorder=5, label='Circles (midpoints)')
    # Cluster labels only (no marker at center)
    ...
    ax.set_title("Bird’s‑eye — Circles (green ✕) and cluster labels (T#)")
