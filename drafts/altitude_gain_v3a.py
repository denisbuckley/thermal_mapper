...
"""
altitude_gain_v3a.py
=================================
Altitude-only detector with **CSV export (explicit altitude naming) + DataFrame** output.
- Parses via igc_utils (parse_igc / compute_derived / detect_tow_segment)
- Smooths altitude, computes vario (m/s)
- Detects sustained climb segments by thresholds
- For each climb: gain, duration, mean m/s, time range, idx range, midpoint lat/lon
- Exports results to: outputs/altitude_climbs_<yymmddhhmmss>.csv
- Returns a pandas DataFrame from main() for in-Python use (if imported)
- Plots (unchanged):
    * Altitude vs time (climbs shaded)
    * Bird’s‑eye map: track (green), climbs (orange), midpoint markers 'C1','C2',…
- Logs console to altitude_gain_<yymmddhhmmss>.txt
"""
...
def detect_climb_segments(df: pd.DataFrame,
                          ...
                          allow_gap_seconds: float = 6.0) -> Tuple[List[Tuple[int,int,float,float,float]], np.ndarray, pd.Series, str, float]:
    """
...
def segments_to_dataframe(df: pd.DataFrame, segments: List[Tuple[int,int,float,float,float]]) -> pd.DataFrame:
    rows = []
...
def plot_map_with_climbs(df: pd.DataFrame, segments):
    ...
    # Midpoint markers + labels
    for idx, (s, e, gain, dur, mean_mps) in enumerate(segments, start=1):
        mid = (s + e) // 2
        lon_m, lat_m = df["lon"].iloc[mid], df["lat"].iloc[mid]
        ax.scatter(lon_m, lat_m, marker='x', s=80, c='k', zorder=6)
        ax.annotate(f"C{idx}", (lon_m, lat_m),
                    textcoords="offset points", xytext=(6, 6),
                    fontsize=9, weight='bold')

    ax.set_title("Bird’s‑eye view — Sustained Climb Segments\nTrack=green, Climbs=orange, Midpoints=black X")
...
def main():
    ...
    print(f"[alt v3a] Parsing IGC: {path}")
    df = parse_igc(path)
    if len(df) == 0:
        print("[alt v3a] No rows parsed; aborting."); return
    ...
    try:
        ...
        print(f"[alt v3a] Tow detect error: {e}")
        tow = None
    if tow and isinstance(tow, tuple) and len(tow) == 2:
        s,e = tow
        print(f"[alt v3a] Tow detected: {s}→{e}. Excluding tow.")
        df = df.iloc[e+1:].reset_index(drop=True)
        df = ensure_time_sec(df)
    else:
        print("[alt v3a] Tow not clearly detected; using full trace.")
    print(f"[alt v3a] Points after tow trim: {len(df)}")

    # Detect climbs
    segments, vario, alt_s, alt_col, dt = detect_climb_segments(df)
    print(f"[alt v3a] Cadence ≈ {dt:.2f} s/fix")
    print(f"[alt v3a] Climb segments found: {len(segments)}")

    # Build results DataFrame
    climbs_df = segments_to_dataframe(df, segments)
    total_gain = climbs_df["gain_m"].sum() if not climbs_df.empty else 0.0
    print(f"[alt v3a] Total climb across segments: +{total_gain:.0f} m")

    # Ensure outputs folder and write CSV
    out_dir = Path("outputs")
    out_dir.mkdir(parents=True, exist_ok=True)
    csv_path = out_dir / f"altitude_climbs_{ts}.csv"
    climbs_df.to_csv(csv_path, index=False)
    print(f"[alt v3a] Wrote climbs CSV: {csv_path}   ({len(climbs_df)} rows)")
    ...
    # If imported elsewhere, return the DataFrame
    return climbs_df
