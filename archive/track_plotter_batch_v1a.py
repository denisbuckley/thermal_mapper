...
Usage examples:
  python track_plotter_batch_v1.py
  python track_plotter_batch_v1.py --glob "105*" --limit 20
  python track_plotter_batch_v1.py --stems 105351 105354 --show
  python track_plotter_batch_v1.py --save-dir outputs/custom_png --dpi 200 --overwrite
"""
...
# ---------------- stats from matched_clusters.csv ----------------

def matched_summary_text(run_dir: Path) -> str:
    """
    Build a compact stats string from matched_clusters.csv (if present).
    Expected columns (case-insensitive): climb_rate_ms, alt_gain_m, duration_s,
    dist_m, overlap_frac. Missing columns show as 'n/a'.
    """
    ...
    def stat(colname, fn=np.nanmean, fmt=".2f"):
        c = get(colname)
        if not c: return "n/a"
        s = pd.to_numeric(m[c], errors="coerce").dropna()
        if s.empty: return "n/a"
        return format(fn(s), fmt)

    mean_cr   = stat("climb_rate_ms")
    p90_cr    = stat("climb_rate_ms", fn=lambda s: np.percentile(s, 90))
    mean_gain = stat("alt_gain_m", fmt=".0f")
    mean_dur  = stat("duration_s", fmt=".1f")
    mean_dist = stat("dist_m", fmt=".1f")
    mean_ovl  = stat("overlap_frac", fmt=".2f")

    lines = [
        f"matches: {len(m)}",
        f"mean climb: {mean_cr} m/s",
        f"p90 climb: {p90_cr} m/s",
        f"avg gain: {mean_gain} m",
        f"avg dur: {mean_dur} s",
        f"avg dist: {mean_dist} m",
        f"overlap: {mean_ovl}",
    ]
    return "\n".join(lines)
...
def climb_segments(track: pd.DataFrame):
    ...
    if track["time"].notna().any():
        # use astype('int64') instead of deprecated .view
...
def render_one(stem: str, run_dir: Path, out_png: Path, show: bool = False, dpi: int = 160):
    ...
    # thick track lines (keep colors as before)
    ...
    # overlays: draw squares first, circles after (bigger to enclose)
    ...
    # ---- Weglide link (bottom-left, outside axes) ----
    weglide_url = f"https://www.weglide.org/flight/{stem}"
    link_artist = plt.gcf().text(0.01, 0.01, f"Weglide: {weglide_url}",
                                 transform=plt.gcf().transFigure,
                                 fontsize=10, color="tab:blue",
                                 ha="left", va="bottom")
    # set_url makes it clickable ONLY in vector formats (SVG/PDF).
    try:
        link_artist.set_url(weglide_url)
    except Exception:
        pass

    # ---- Stats box (bottom-right, outside axes, vertical lines) ----
    ...
    plt.gcf().savefig(out_png, dpi=dpi, bbox_inches="tight")  # tight to include the outside text boxes
...
def main():
    ...
    # if not showing, use a non-interactive backend
    if not args.show:
