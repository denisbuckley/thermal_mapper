...
# =========================
# Circles-only detector (strict ~360°, time-based cap) + plotting
# =========================
...
def _sec_per_fix(df: pd.DataFrame) -> float:
    """Estimate logger cadence in seconds per fix using median dt; fallback=1.0s."""
    if 'sec' not in df.columns:
        s = (df['time'] - df['time'].iloc[0]).dt.total_seconds().astype(float)
    else:
        s = df['sec']
    ds = s.diff().dropna()
    dt = float(ds.median()) if not ds.empty else 1.0
    if dt <= 0 or not (dt == dt):  # NaN check
        dt = 1.0
    return dt

def find_circles_360_timecap(df: pd.DataFrame,
...
def plot_circles(df: pd.DataFrame, circles):
    ...
    ax.set_title("Circles-only detection (360°, time-capped)")
...
def main():
    ...
    print(f"[circles-360-timecap] Parsing IGC: {path}")
    df = parse_igc(path)
    ...
    if tow and isinstance(tow, tuple) and len(tow) == 2:
        s, e = tow
        print(f"[circles-360-timecap] Tow detected: {s}→{e}. Excluding tow.")
        df = df.iloc[e+1:].reset_index(drop=True)
    else:
        print("[circles-360-timecap] Tow not clearly detected; using full trace.")

    circles = find_circles_360_timecap(df)
    print(f"[circles-360-timecap] Logger cadence ≈ {_sec_per_fix(df):.2f} s/fix")
    print(f"[circles-360-timecap] Detected {len(circles)} circles")
    for (s, e, cum) in circles:
        t0, t1 = df.loc[s, "time"], df.loc[e, "time"]
        dur_s = (t1 - t0).total_seconds()
        print(f"  circle: idx {s}->{e}, samples {e-s}, dur {dur_s:.1f}s, heading {cum:.1f}°")
