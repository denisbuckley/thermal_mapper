...
# -*- coding: utf-8 -*-
"""
batch_match_igc_v3e.py — per‑flight matching with **plot suppression**

Changes vs v3d:
  • Suppresses interactive plots when calling detectors:
        - sets env MPLBACKEND=Agg and BATCH_MODE=1
        - tries adding "--no-show"; if detector doesn't accept it, retries without
  • Everything else identical to v3d (schema-aware pickup; robust logging).

Usage:
  python batch_match_igc_v3e.py --igc_subset-dir ./igc_subset --alt-script altitude_gain_v3g.py --circ-script circles_clean_v2c.py --outputs-dir outputs
"""
...
def run_cli_no_plots(script: Path, igc: Path, cwd: Path) -> str:
    env = os.environ.copy()
    env["MPLBACKEND"] = "Agg"
    env["BATCH_MODE"] = "1"
    # First try with --no-show
    cmd = [sys.executable, str(script), str(igc), "--no-show"]
    r = subprocess.run(cmd, cwd=str(cwd), capture_output=True, text=True, env=env)
    if r.returncode == 0:
        return "ok(noshow)"
    if "unrecognized arguments: --no-show" in (r.stderr or ""):
        # Retry without the flag
        r2 = subprocess.run([sys.executable, str(script), str(igc)], cwd=str(cwd), capture_output=True, text=True, env=env)
        if r2.returncode == 0:
            return "ok"
        return f"rc={r2.returncode} stderr={r2.stderr.strip()[:200]}"
...
def main():
    ...
    ap.add_argument("--igc_subset-dir", default=None)
    ...
    with open(log_path, "w", encoding="utf-8") as log:
        ...
        for igc in igc_files:
            ...
            alt_res = run_cli_no_plots(alt_script, igc, cwd=script_dir)
            circ_res = run_cli_no_plots(circ_script, igc, cwd=script_dir)
