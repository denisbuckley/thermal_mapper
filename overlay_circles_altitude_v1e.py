...
"""
overlay_circles_altitude_v1e.py
--------------------------------
Working overlay with:
  • Raw circle detections (green ✕)
  • Circle-based thermal clusters (black ✕)
  • Altitude-based thermals (orange ✕ with T# labels)
Also writes a debug snapshot to outputs/overlay_debug_<timestamp>.txt

Fixes from previous attempts:
- Proper imports (no stray pd renames).
- Uses .total_seconds() for timedelta → seconds.
- Consistent variable names (circ_clusters_df).
- Actually calls plot + writes debug file.
"""
...
# -------------------- Tuning: CIRCLES --------------------
C_MIN_ARC_DEG      = 300.0   # deg to count as a full circle
C_SMOOTH_S         = 3.0     # heading smoothing (sec)
C_MAX_WIN_SAMPLES  = 2400    # safety window in samples
C_MIN_DIR_RATIO    = 0.65    # majority direction fraction

CLUSTER_EPS_M      = 1500.0  # circle-cluster spatial eps (m)
CLUSTER_GAP_S      = 900.0   # temporal bridge (s)
# --------------------------------------------------------

# -------------------- Tuning: ALTITUDE ------------------
A_SMOOTH_S         = 9.0     # altitude smoothing (sec)
A_MIN_RATE_MPS     = 0.5     # min climb rate (m/s)
A_MIN_DUR_S        = 60.0    # min duration (s)
A_MIN_GAIN_M       = 60.0    # min altitude gain (m)

A_CLUSTER_EPS_M    = 2000.0  # (not used here — we use valley merge instead)
A_CLUSTER_GAP_S    = 1200.0
A_ALT_DROP_M       = 180.0   # valley merge absolute drop (m)
A_ALT_DROP_FRAC    = 0.30    # or <30% drop of previous peak
# --------------------------------------------------------
            ...
            while cum_abs >= min_arc_deg:
                ...
                if ratio >= min_dir_ratio:
                    ...
                    if e_idx - s_idx >= 2:
                        ...
                        last_emit_j = j
                # discharge one full circle’s worth
                if pos_abs >= neg_abs:
...
def circle_clusters(circ_df: pd.DataFrame,
    ...
    # adjacency under eps + temporal overlap / small gap
...
def write_debug_file(circles, circ_clusters_df: pd.DataFrame, alt_clusters_df: pd.DataFrame, dt: float):
    out_dir = Path("outputs"); ensure_dir(out_dir)
    ...
    print(f"[overlay] Wrote debug file: {fn}")
