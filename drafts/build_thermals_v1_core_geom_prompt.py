#!/usr/bin/env python3
"""
build_thermals_v1_core_geom_prompt.py
-------------------------------------
Aggregate per-flight matched cluster points into persistent thermal *core* locations
using DBSCAN (haversine). Centers are computed over **core points** with a user-selectable
estimator. In IDE runs (no args), you'll be PROMPTED for the center estimator with
default **median**; in CLI runs, pass --center-estimator to override non-interactively.

Center estimators:
- geomedian: geometric median (Weiszfeld)
- median:    coordinate-wise median (DEFAULT when prompted)
- medoid:    actual point minimizing total GC distances to all others

Outputs/paths kept same as before.
"""
...
def parse_args():
    ap = argparse.ArgumentParser(description="Group matched clusters into persistent thermal CORE locations with interactive center-estimator prompt for IDE runs.")
    # tunings (set to None so we can prompt in IDE runs)
    ...
    # center-estimator: default=None triggers an IDE prompt; CLI can still pass a value
    ap.add_argument("--center-estimator", choices=["geomedian","median","medoid"], default=None,
                    help="Estimator for cluster center derived from core points. If omitted, you'll be prompted (default shown: median).")
...
def main():
    ...
    center_estimator = _ask(
        args.center_estimator,
        "Center estimator (geomedian|median|medoid)",
        DEF_CENTER,
        str,
        validator=lambda s: s in valid_centers
    ).lower()
    ...
    with open(logp, "a") as lf:
        log("=== build_thermals_v1_core_geom_prompt start ===", lf)
