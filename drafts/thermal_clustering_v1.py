...
"""
thermal_clustering_v1.py
Baseline pipeline to:
1) Parse IGC (uses your existing parse_igc / compute_derived / detect_tow_segment)
2) Detect full 360° circles (tuned duration/turn-rate; auto cadence)
3) Cluster circles into thermals (spatial + temporal)
4) Plot: track (green), circle spans (blue with red endpoints), thermal centers (black X)
5) Log all console output to a timestamped file alongside the run
"""
...
def haversine_m(lat1, lon1, lat2, lon2):
    ...
    dlat = lat2 - lat1
    dlon = lon1 * 0 + (lon2 - lon1)  # broadcast-safe
...
def fit_circle_xy(x, y):
    """Algebraic circle fit (Kåsa). Returns xc, yc, r. Falls back to centroid if singular."""
...
def find_circles_360_tuned(df: pd.DataFrame,
    ...
    """
    Detect ~360° circles with:
      - consistent direction,
      - duration window (converted via cadence),
      - mean |turn rate| guard to reject jitter.
    Returns list of (start_idx, end_idx, arc_deg, dur_s, mean_abs_turnrate_dps) and sec_per_fix.
    """
    df = _ensure_sec(df)
    n = len(df)
    if n < 3:
...
def cluster_circles_spatiotemporal(df: pd.DataFrame,
    ...
    """
    Single-link (BFS) clustering by proximity and time overlap/adjacency.
    Two circle centers connect if:
      - distance(center_i, center_j) <= merge_dist_m, AND
      - time windows overlap or are within max_time_gap_s.
...
def main():
    ...
    thermals = cluster_circles_spatiotemporal(df, circles,
                                             merge_dist_m=350.0,   # tweakable
                                             max_time_gap_s=20*60, # 20 min window
