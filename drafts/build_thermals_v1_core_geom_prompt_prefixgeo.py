#!/usr/bin/env python3
"""
build_thermals_v1_core_geom_prompt_prefixgeo.py
-----------------------------------------------
Same as the "final" script, but **forces popup order in common map renderers**
that alphabetize properties by **prefixing GeoJSON property keys** with
two-digit order indices (01_, 02_, ...).

- CSV columns: unchanged (no prefixes), in your requested order.
- GeoJSON properties: prefixed keys to lock the on-map info box order.
- Membership CSV unchanged.

Order used (and corresponding prefixed keys):
...
# Build prefixed GeoJSON order
GEO_KEYS = [f"{i:02d}_{k}" for i, k in enumerate(CSV_ORDER, start=1)]
...
def parse_args():
    ap = argparse.ArgumentParser(description="Thermal core waypoints with forced GeoJSON popup order (prefixed keys).")
...
def aggregate_clusters(df: pd.DataFrame, labels: np.ndarray, core_mask: np.ndarray,
    ...
    for lab, grp in df.groupby("__label"):
        ...
        rows.append({
            "cluster_label": int(lab),  # internal only; not exported to CSV/GeoJSON
    ...
    tmp.insert(CSV_ORDER.index("wp_id") if "wp_id" in CSV_ORDER else 15, "wp_id", np.arange(len(tmp)))
...
def main():
    ...
    with open(logp, "a") as lf:
        log("=== build_thermals_v1_core_geom_prompt_prefixgeo start ===", lf)
        ...
        # Persist artifacts
        if not args.dry_run:
            csv_df.to_csv(out_csv, index=False)                  # ordered, no prefixes
            write_geojson_prefixed(csv_df, out_geo)              # prefixed keys for popup order
