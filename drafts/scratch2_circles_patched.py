...
# Early gate for circles-only flow (does not alter default behaviour)
if __name__ == '__main__' and '--circles-only' in sys.argv:
    # choose first non-flag argument as path, else default
    path_arg = None
    for a in sys.argv[1:]:
        if not a.startswith('-'):
            path_arg = a
            break
    try:
        path = path_arg or DEFAULT_IGC_PATH
    except NameError:
        path = path_arg or '2020-11-08 Lumpy Paterson 108645.igc'
    _run_circles_only(path)
    import sys as _sys
    _sys.exit(0)
...
# =========================
# Circles-only (basic) detector
# =========================

def find_circles_basic(df: pd.DataFrame,
                       min_circle_heading_deg: float = 300.0,
                       max_len_samples: int = 1000):
    """Detect circles purely by accumulated heading change.
    Returns a list of (start_idx, end_idx, heading_change_deg).
    No altitude/climb filtering; strictly heading accumulation.
    """
    circles = []
    n = len(df)
    i = 0
    while i < n - 2:
        ...
        while j < n and (j - i) < max_len_samples:
            ...
            if abs(cum) >= min_circle_heading_deg:
                circles.append((i, j, cum))
                i = j + 1  # jump to after the circle
                break
            j += 1
        else:
            i += 1
...
def _run_circles_only(path: str):
    print(f"[circles-only] Parsing IGC: {path}")
    df = parse_igc(path)
    df = compute_derived(df)
    # Tow trim using existing tow detector if present
    tow_seg = None
    try:
        tow_seg = detect_tow_segment(df)
    except Exception:
        tow_seg = None
    if tow_seg and isinstance(tow_seg, tuple) and len(tow_seg) == 2:
        s, e = tow_seg
        print(f"[circles-only] Tow detected: {s}→{e}. Excluding tow.")
        df = df.iloc[e+1:].reset_index(drop=True)
    else:
        print("[circles-only] Tow not clearly detected; using full trace.")

    circles = find_circles_basic(df)
    print(f"[circles-only] Detected {len(circles)} circles")
    for (s, e, hdg) in circles[:50]:
        t0, t1 = df.loc[s, 'time'], df.loc[e, 'time']
        print(f"  circle: idx {s}->{e}, span {(e-s)} samp, {t0.time()}→{t1.time()}, heading {hdg:.1f}°")
    if len(circles) > 50:
        print(f"  ... ({len(circles)-50} more)")