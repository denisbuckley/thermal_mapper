...
# =========================
# Circles-only detector tuned for ~18–45 s circles + logfile output
# (with df['time'] -> datetime fix) + parse_igc debug
# =========================
...
def main():
    ...
    try:
        print(f"[debug] parse_igc rows={len(df)}, cols={list(df.columns)}")
        print("[debug] head(5):")
        print(df.head(5))
    except Exception as e:
        print(f"[debug] parse_igc inspection failed: {e}")

    # Ensure df['time'] is datetime-like for downstream .dt usage
    try:
        df['time'] = pd.to_datetime(df['time'], errors='coerce')
    except Exception as e:
        print(f"[circles-360-tuned] to_datetime coercion issue: {e}")
    ...
    try:
        ...
        tow = None
        print(f"[circles-360-tuned] Tow detect error: {e}")
