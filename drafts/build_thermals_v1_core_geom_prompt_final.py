#!/usr/bin/env python3
"""
build_thermals_v1_core_geom_prompt_final.py
-------------------------------------------
...
Exports the waypoint info box (CSV + GeoJSON properties) in EXACT order:
  strength_mean_core
  radius_m
  core_count
  border_count
...
`cluster_label` is omitted from the waypoint CSV/GeoJSON but retained in membership CSV.
"""
    ...
    for _, r in df.iterrows():
        ...
        for k in EXPORT_ORDER:
            ...
            if isinstance(v, float) and (np.isnan(v) or np.isinf(v)):
                ...
                # Ensure basic Python types for JSON
...
def main():
    ...
    with open(logp, "a") as lf:
        log("=== build_thermals_v1_core_geom_prompt_final start ===", lf)
