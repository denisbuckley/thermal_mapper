...
"""
circles_clean_v2b.py
====================
Fix: detect *multiple* circles within one continuous turning run.
- We continue advancing j while the turn keeps roughly the same sign.
- Every time |cum_turn| crosses MIN_ARC_DEG, we emit a circle *without* jumping
  all the way past j+1; instead we reduce cum_turn by 360 (keep remainder) and
  keep searching inside the same turning segment. This yields many circles per
  thermal instead of one.
- Still uncluttered plot: Track (green), raw circles (green ✕), cluster centers (black ✕).

Exports CSVs for circles, clusters, and cluster-to-cluster separations.
"""
...
# ---------------- Tunables ----------------
MIN_ARC_DEG = 300.0          # full circle threshold (deg)
HEADING_SMOOTH_S = 3.0       # heading smoothing (s)
MAX_LEN_SAMPLES = 2000       # safety cap on segment length in samples

EPS_M = 1500.0               # spatial clustering radius (m)
...
def haversine_m(lat1, lon1, lat2, lon2):
    ...
    dlat, dlon = lat2 - lon1, lon2 - lon1  # BUG intentional? No, fix below
...
def detect_circles(df: pd.DataFrame,
    ...
    - Grow a segment while turn keeps mostly the same sign.
    - Each time |cum| crosses threshold, emit a circle covering the
      last threshold span; subtract threshold (keep remainder) and continue.
    """
    ...
    i = 0
    while i < n-3:
        # start new potential turning run
        j = i + 1
        last_h = hdg_s[i]
        cum = 0.0
        sign = 0
        # where last circle threshold was crossed (index)
        last_cross_j = i
        while j < n and (j - i) < max_len_samples:
            ...
            if sign == 0:
                sign = sgn
            # If turn direction flips significantly, end this run
            elif sgn != sign and abs(cum) > (0.25 * min_circle_heading_deg):
                break

            cum += d

            # Emit *every* time we cross threshold
            while abs(cum) >= min_circle_heading_deg:
                # choose segment from last_cross_j → j
                s_idx = last_cross_j
                e_idx = j
                if e_idx - s_idx >= 2:
                    circles.append((s_idx, e_idx, sign * min_circle_heading_deg))
                # keep remainder beyond one full circle
                cum -= sign * min_circle_heading_deg
                last_cross_j = j  # next circle starts accumulating from here

            j += 1

        # advance start index forward; if we emitted something, move to last_cross_j
        i = max(j, last_cross_j + 1)
...
def clusters_from_circles(circles_df: pd.DataFrame, eps_m: float = EPS_M, max_gap_s: float = MAX_GAP_S):
    ...
    # adjacency rule: within eps AND temporally close/overlapping
...
def main():
    ...
    print(f"[circles-clean v2b] Parsing IGC: {igc_file}")
    df = parse_igc(igc_file)
    if df.empty:
        print("[circles-clean v2b] No rows parsed; aborting."); return
    df = compute_derived(df); df = ensure_time_sec(df)

    try:
        tow = detect_tow_segment(df)
    except Exception as e:
        print(f"[circles-clean v2b] Tow detect error: {e}"); tow = None
    if isinstance(tow, tuple) and len(tow) == 2:
        s,e = tow
        print(f"[circles-clean v2b] Tow detected: {s}→{e}. Excluding tow.")
        df = df.iloc[e+1:].reset_index(drop=True); df = ensure_time_sec(df)
    else:
        print("[circles-clean v2b] Tow not clearly detected; using full trace.")
    print(f"[circles-clean v2b] Points after tow trim: {len(df)}")

    circles, dt = detect_circles(df, MIN_ARC_DEG, MAX_LEN_SAMPLES, HEADING_SMOOTH_S)
    print(f"[circles-clean v2b] Cadence ≈ {dt:.2f} s/fix | circles detected: {len(circles)}")
    ...
    ts2 = ts
    (out / f"circles_turns_{ts2}.csv").write_text(circ_with_cluster.to_csv(index=False))
    (out / f"circles_clusters_{ts2}.csv").write_text(clusters_df.to_csv(index=False))
