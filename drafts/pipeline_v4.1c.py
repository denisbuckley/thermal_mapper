...
def _df_stats(df, cols):
    out = {}
    for c in cols:
        if c in df.columns:
            out[c] = _series_stats(df[c])
    return out

def build_pipeline_summary(circles, cc, alts, matches):
    """Return a nested dict with counts and distributions across artifacts."""
    summary = {
        "counts": {
            "circles": int(len(circles)) if circles is not None else 0,
            "circle_clusters": int(len(cc)) if cc is not None else 0,
            "altitude_clusters": int(len(alts)) if alts is not None else 0,
            "matched_clusters": int(len(matches)) if matches is not None else 0,
        },
        "circles": {},
        "circle_clusters": {},
        "altitude_clusters": {},
    ...
    # Circles: distributions for climb rate, gain, duration, radius, bank
    if circles is not None and len(circles):
        summary["circles"] = _df_stats(
            circles,
            ["climb_rate_ms", "alt_gain_m", "duration_s", "turn_radius_m", "bank_angle_deg"]
        )

    # Circle clusters: typical per-spot stats
    if cc is not None and len(cc):
        summary["circle_clusters"] = _df_stats(
            cc,
            ["climb_rate_ms", "climb_rate_ms_median", "alt_gain_m_mean", "duration_s_mean", "n_circles"]
        )

    # Altitude clusters: climb-only segments
    if alts is not None and len(alts):
        summary["altitude_clusters"] = _df_stats(
            alts,
            ["climb_rate_ms", "alt_gain_m", "duration_s"]
        )

    # Matched clusters: use alt-side metrics + match diagnostics
    if matches is not None and len(matches):
        summary["matched_clusters"] = _df_stats(
            matches,
            ["climb_rate_ms", "alt_gain_m", "duration_s", "dist_m", "time_overlap_s", "overlap_frac"]
        )

    return summary

def write_summary_files(run_dir: Path, summary: dict):
    """Write summary.json and summary.txt in run_dir."""
    # JSON
    (run_dir / "summary.json").write_text(json.dumps(summary, indent=2), encoding="utf-8")

    # TXT (human-readable)
    def line_for(group, metric):
        g = summary.get(group, {})
        m = g.get(metric, {})
        if not m or m.get("count", 0) == 0:
            return f"{group}.{metric}: n=0"
        return (f"{group}.{metric}: n={m['count']} "
                f"min={m['min']:.3f} p25={m['p25']:.3f} p50={m['p50']:.3f} "
                f"p75={m['p75']:.3f} p90={m['p90']:.3f} max={m['max']:.3f} mean={m['mean']:.3f}")

    lines = []
    counts = summary.get("counts", {})
    lines.append("=== COUNTS ===")
    for k in ("circles", "circle_clusters", "altitude_clusters", "matched_clusters"):
        lines.append(f"{k}: {counts.get(k, 0)}")
    lines.append("")

    blocks = [
        ("circles", ["climb_rate_ms","alt_gain_m","duration_s","turn_radius_m","bank_angle_deg"]),
        ("circle_clusters", ["climb_rate_ms","climb_rate_ms_median","alt_gain_m_mean","duration_s_mean","n_circles"]),
        ("altitude_clusters", ["climb_rate_ms","alt_gain_m","duration_s"]),
        ("matched_clusters", ["climb_rate_ms","alt_gain_m","duration_s","dist_m","time_overlap_s","overlap_frac"]),
    ]
    for grp, metrics in blocks:
        lines.append(f"=== {grp.upper()} ===")
        for met in metrics:
            lines.append(line_for(grp, met))
        lines.append("")
...
def main() -> int:
    ...
    if len(matches):
        ...
        payload["summary"] = inline
    match_json.write_text(json.dumps(payload, indent=2), encoding="utf-8")

    # ---- NEW: write overall pipeline summaries (json + txt) ----
    summary = build_pipeline_summary(circles, cc, alts, matches)
    write_summary_files(run_dir, summary)

    # Log headline counts
    counts = summary.get("counts", {})
    log_line = (f"SUMMARY counts: circles={counts.get('circles',0)}, "
                f"circle_clusters={counts.get('circle_clusters',0)}, "
                f"altitude_clusters={counts.get('altitude_clusters',0)}, "
                f"matched_clusters={counts.get('matched_clusters',0)}")
    logf_write(logf, log_line)
    print()
