...
"""
	•	C_MIN_DIR_RATIO = 0.75
	•	C_MAX_WIN_SAMPLES = 500
	•	CLUSTER_EPS_M = 1100.0
	•	CLUSTER_GAP_S = 600.0
	•	Turn-rate keep window = [6, 18] deg/s (new post-filter)
	•	Radius keep window = [50, 400] m (new post-filter)
	•	MATCH_EPS_M = 2500.0
	•	MATCH_GAP_S = 1200.0
compare_circles_altitude_v2c.py
-------------------------------
Purpose
    Produce a table that *matches* altitude-based thermal clusters to nearest
    circle-based clusters (spatial + temporal), to evaluate agreement.

What this script does
    1) Parses IGC and removes tow (if detected).
    2) Detects circles and clusters them (same logic as overlay).
    3) Detects altitude-based thermals (same logic as overlay).
    4) For each altitude cluster, finds the nearest circle cluster within MATCH_EPS_M
       and measures time overlap/gap.
    5) Exports CSV to outputs/compare_circles_altitude_<yymmddhhmmss>.csv
    6) Prints a compact preview to console.

Why this version (“v2c”)
    - Same dtype/timedelta hardening as overlay.
    - Self-contained detectors inside this script.
    - Clearer output columns for quick inspection.

Inputs
    - IGC path is set in main(). Requires igc_utils.py (parse_igc, compute_derived, detect_tow_segment).

Tuning
    MATCH_EPS_M  : max distance (m) between altitude center and circle-cluster center to consider a match.
    MATCH_GAP_S  : if no overlap, maximum allowed time gap (s) to still consider a match “nearby in time”.

Outputs
    - CSV with one row per altitude cluster and its best circle-cluster match (if any).

Usage
    - Run directly in PyCharm. Inspect the printed preview and open the CSV for full details.
"""
...
def main():
    ...
    print(f"[compare v2c] Parsing IGC: {igc_file}")
    ...
    if isinstance(tow, tuple) and len(tow) == 2:
        s, e = tow
        print(f"[compare v2c] Tow detected: {s}→{e}. Excluding tow.")
    ...
    ensure_dir(out_dir)
    out_csv = out_dir / f"compare_circles_altitude_{ts_stamp()}.csv"
