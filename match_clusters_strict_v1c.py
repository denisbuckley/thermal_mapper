...
def load_enriched():
    ...
    if not circ or not alti:
        print("[match_strict v1c] Missing enriched inputs. Run enrichment first.")
...
def main():
    ...
    print(f"[match_strict v1c] Candidate pairs: {len(cands)}")

    pairs = greedy_assign(cands)
    print(f"[match_strict v1c] Strict 1:1 matches: {len(pairs)}")
    ...
    print(f"[match_strict v1c] Wrote {out_csv}")
    ...
    if not M.empty:
        ...
        print("[match_strict v1c] No matches under current thresholds. Consider widening EPS_M or lowering MIN_OVL_FRAC.")
