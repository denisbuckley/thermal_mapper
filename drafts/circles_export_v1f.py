...
def plot_map_with_circles_and_clusters(df: pd.DataFrame, circles, clusters_df: pd.DataFrame, dt: float):
    ...
    for _, row in big.iterrows():
        ax.scatter(row["center_lon"], row["center_lat"], marker='D', s=120, c='k', zorder=6)
        ax.annotate(f"C{int(row['cluster_id'])}",
                    (row["center_lon"], row["center_lat"]),
                    textcoords="offset points", xytext=(6, 6),
                    fontsize=10, weight='bold')
    add_params_box(ax, dt)
    ax.set_title("Bird’s‑eye — Circles and C-clusters (from circles only) (Track=green, Circles=blue, Centers=black X)")
