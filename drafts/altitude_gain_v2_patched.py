...
altitude_gain_v2.py
=================================
Altitude-only detector with coordinates & labeled map.
...
- Prints for each climb: gain, duration, mean m/s, time range, idx range, **lat/lon midpoint**
- Plots:
...
def main():
    ...
    df = parse_igc(path)
    if len(df) == 0:
        print("[alt v2] No rows parsed; aborting."); return
    ...
    try:
        ...
        print(f"[alt v2] Tow detect error: {e}")
        tow = None
    if tow and isinstance(tow, tuple) and len(tow) == 2:
        s,e = tow
        print(f"[alt v2] Tow detected: {s}→{e}. Excluding tow.")
        df = df.iloc[e+1:].reset_index(drop=True)
        df = ensure_time_sec(df)
    else:
        print("[alt v2] Tow not clearly detected; using full trace.")
    print(f"[alt v2] Points after tow trim: {len(df)}")

    segments, vario, alt_s, alt_col, dt = detect_climb_segments(df)
    print(f"[alt v2] Cadence ≈ {dt:.2f} s/fix")
    print(f"[alt v2] Climb segments found: {len(segments)}")
    total_gain = 0.0
    for i,(s,e,gain,dur,mean_mps) in enumerate(segments, start=1):
        mid = (s + e) // 2
        lat, lon = df['lat'].iloc[mid], df['lon'].iloc[mid]
        total_gain += gain
        print(f"  Climb {i}: +{gain:.0f} m  dur={dur:.0f}s  mean={mean_mps:.2f} m/s  "
              f"{df['time'].iloc[s].time()}→{df['time'].iloc[e].time()}  "
              f"idx={s}→{e}  coords=({lat:.6f},{lon:.6f})")
    print(f"[alt v2] Total climb across segments: +{total_gain:.0f} m")
