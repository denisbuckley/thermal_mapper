...
# =========================
# Circles-only detector (strict ~360°, robust advance) + plotting
# =========================
...
def find_circles_360_fixed(df: pd.DataFrame,
                            min_circle_heading_deg: float = 340.0,
                            max_circle_heading_deg: float = 380.0,
                            tiny_step_deg: float = 0.2,
                            max_len_samples: int = 240):
    """
    Detect discrete full circles by accumulating signed heading deltas (~360°)
    in a single direction (no reversals). Guaranteed forward progress: O(n).
    Returns list of (start_idx, end_idx, cum_heading_deg).
    """
    n = len(df)
    if n < 3:
        return []
    ...
    i = 0
    while i < n - 1:
        # Find first meaningful step to establish direction
        while i < n - 1:
            ...
            break  # no more meaningful motion

        # Accumulate from this start
        start = i
        cum = 0.0
        j = i + 1
        while j < n and (j - start) <= max_len_samples:
            ...
            cum += step
            ac = abs(cum)
            if ac >= min_circle_heading_deg and ac <= max_circle_heading_deg:
                circles.append((start, j, cum))
                # advance start to end of this circle
                i = j
                break
            j += 1
        else:
            # ran out or too long without completion; advance to j to guarantee progress
            i = j
            continue

        # if we recorded a circle, loop continues with i already set to j
...
def plot_circles(df: pd.DataFrame, circles):
    ...
    ax.set_title("Circles-only detection (strict 360°, robust)")
...
def main():
    # Use the baked-in default path from your script if available
    try:
        path = DEFAULT_IGC_PATH
    except NameError:
        path = "../2020-11-08 Lumpy Paterson 108645.igc"
    print(f"[circles-360-fix] Parsing IGC: {path}")
    df = parse_igc(path)
    df = compute_derived(df)

    # Tow trim via your existing detector
    try:
        tow = detect_tow_segment(df)
    except Exception:
        tow = None
    if tow and isinstance(tow, tuple) and len(tow) == 2:
        s, e = tow
        print(f"[circles-360-fix] Tow detected: {s}→{e}. Excluding tow.")
        df = df.iloc[e+1:].reset_index(drop=True)
    else:
        print("[circles-360-fix] Tow not clearly detected; using full trace.")

    circles = find_circles_360_fixed(df)
    print(f"[circles-360-fix] Detected {len(circles)} circles")
    for (s, e, cum) in circles:
        t0, t1 = df.loc[s, "time"], df.loc[e, "time"]
        print(f"  circle: idx {s}->{e}, samples {e-s}, {t0.time()}→{t1.time()}, heading {cum:.1f}°")
