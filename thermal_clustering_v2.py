...
"""
thermal_clustering_v2.py
Refinements over v1:
- Spatio-temporal clustering (same core)
- New filters: min circles per thermal AND min duration (seconds)
- Plot labels (T1, T2, …) at thermal centers
- Draw mean-radius rings for each thermal
- Logging to thermal_cluster_<yymmddhhmmss>.txt
Requires: igc_utils.py (re-exporting parse_igc / compute_derived / detect_tow_segment)
"""
...
def cluster_circles_spatiotemporal(df: pd.DataFrame,
    ...
    """
    Single-link clustering with spatial AND temporal adjacency.
    Filters clusters by:
      - at least min_circles_per_cluster circles
      - total time span >= min_duration_s
    """
    if not circles:
        return []
...
def _ring_points(lat_c, lon_c, radius_m, npts=90):
    """Return lat/lon points approximating a circle of given radius around center."""
    if radius_m <= 0:
        return np.array([lon_c]), np.array([lat_c])
    # build ring in local XY then convert back
...
def plot_circles_and_thermals(df: pd.DataFrame, circles: list, thermals: list):
    ...
    # thermals: center "X" + radius ring + label
    for idx, th in enumerate(thermals, start=1):
        ax.scatter(th['center_lon'], th['center_lat'], marker='x', s=80, c='k', zorder=6)
        # radius ring (mean radius)
        ring_x, ring_y = _ring_points(th['center_lat'], th['center_lon'], th['mean_radius_m'])
        ax.plot(ring_x, ring_y, lw=1.2, alpha=0.6)  # default axes color
        # label
        ax.annotate(f"T{idx}",
                    (th['center_lon'], th['center_lat']),
                    textcoords="offset points",
                    xytext=(6, 6),
                    fontsize=9, weight='bold')

    ax.set_title("Thermals (labels+radius) and Circles\nTrack=green, Circles=blue/red, Thermal centers=black X")
...
def main():
    ...
    print(f"[thermal v2] Parsing IGC: {path}")
    df = parse_igc(path)

    try:
        df['time'] = pd.to_datetime(df['time'], errors='coerce')
    except Exception as e:
        print(f"[thermal v2] to_datetime error: {e}")
    if len(df) == 0:
        print("[thermal v2] No rows parsed; aborting.")
        return

    df = compute_derived(df)
    print(f"[thermal v2] Derived columns: {list(df.columns)[:12]} ... (total {len(df.columns)})")
    ...
    try:
        ...
        tow = None
        print(f"[thermal v2] Tow detect error: {e}")
    if tow and isinstance(tow, tuple) and len(tow) == 2:
        s, e = tow
        print(f"[thermal v2] Tow detected: {s}→{e}. Excluding tow.")
        df = df.iloc[e+1:].reset_index(drop=True)
    else:
        print("[thermal v2] Tow not clearly detected; using full trace.")
    print(f"[thermal v2] Points after tow trim: {len(df)}")

    # Circles
    circles, dt_fix = find_circles_360_tuned(df)
    print(f"[thermal v2] Cadence ≈ {dt_fix:.2f} s/fix")
    print(f"[thermal v2] Circles detected: {len(circles)}")

    # Clusters with stricter defaults
    thermals = cluster_circles_spatiotemporal(df, circles,
                                              merge_dist_m=350.0,
                                              max_time_gap_s=20*60,
                                              min_circles_per_cluster=3,
                                              min_duration_s=90.0)
    print(f"[thermal v2] Thermals (after filters): {len(thermals)}")
    ...
    if len(circles) > 0:
        ...
        print("[thermal v2] No circles; nothing to plot.")
