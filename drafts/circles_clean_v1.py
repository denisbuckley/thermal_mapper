...
- Detect circles by cumulative heading (≥ 300°), minimal smoothing (3 s).
- Drift-friendly clustering of circles in space/time.
- NO rings, NO blue segments — just the track and green ✕ for circle midpoints.
- Cluster center shown as black ✕ with label T# (uncluttered).
- Same CSV exports as before for easy comparison with altitude pipeline.
...
# ---------------- Tunables (conservative, based on working runs) ----------------
MIN_ARC_DEG = 300.0          # accept circle when cumulative heading reaches this
HEADING_SMOOTH_S = 3.0       # seconds
MAX_LEN_SAMPLES = 1000       # safety cap on window length (in samples)

EPS_M = 1500.0               # meters — spatial radius for clustering
MAX_GAP_S = 900.0            # seconds — temporal bridge for clustering
MIN_CLUSTER_COUNT = 1        # keep even singletons for now
...
def detect_circles(df, min_circle_heading_deg=MIN_ARC_DEG, max_len_samples=MAX_LEN_SAMPLES, smooth_s=HEADING_SMOOTH_S):
...
def circles_to_dataframe(df, circles):
    ...
    for k, (s, e, arc) in enumerate(circles, start=1):
        ...
        chord_m = haversine_m(df['lat'].iloc[s], df['lon'].iloc[s], df['lat'].iloc[e], df['lon'].iloc[e])
...
# ---------------- Simple drift-friendly clustering ----------------
def clusters_from_circles(circles_df, eps_m=EPS_M, max_gap_s=MAX_GAP_S):
    ...
    n = len(df)
    lats, lons = df["lat_mid"].to_numpy(float), df["lon_mid"].to_numpy(float)
    ...
    for i in range(n):
        ...
        while q:
            ...
            for j in adj[k]:
                if not visited[j]: visited[j] = True; q.append(j)
        idx = np.array(members, int)
        w = np.clip(1.0/np.maximum(radius[idx], 1.0), 0.1, None)  # weight tighter circles more
...
def plot_map(df, circles, clusters_df, dt):
    ...
    # Cluster centers (black ✕) with T# label
    big = clusters_df[clusters_df["n"] >= MIN_CLUSTER_COUNT]
    for _, row in big.iterrows():
        ax.scatter(row["center_lon"], row["center_lat"], marker='x', c='k', s=110, linewidths=1.8, zorder=6)
    ...
    ax.set_title("Circles → Thermals (Track=green, Circles=green ✕, Centers=black ✕)")
...
def main():
    ...
    print(f"[circles-clean] Parsing IGC: {igc_file}")
    df = parse_igc(igc_file)
    if df.empty:
        print("[circles-clean] No rows parsed; aborting."); return
    df = compute_derived(df); df = ensure_time_sec(df)

    try:
        tow = detect_tow_segment(df)
    except Exception as e:
        print(f"[circles-clean] Tow detect error: {e}"); tow = None
    if isinstance(tow, tuple) and len(tow) == 2:
        s,e = tow
        print(f"[circles-clean] Tow detected: {s}→{e}. Excluding tow.")
        df = df.iloc[e+1:].reset_index(drop=True); df = ensure_time_sec(df)
    else:
        print("[circles-clean] Tow not clearly detected; using full trace.")
    print(f"[circles-clean] Points after tow trim: {len(df)}")

    circles, dt = detect_circles(df, MIN_ARC_DEG, MAX_LEN_SAMPLES, HEADING_SMOOTH_S)
    print(f"[circles-clean] Cadence ≈ {dt:.2f} s/fix | circles detected: {len(circles)}")
    ...
    print(f"[circles-clean] Wrote CSVs to ./outputs: {f1.name}, {f2.name}, {f3.name}")
