#!/usr/bin/env python3
"""
build_thermals_v1_core.py
-------------------------
Aggregate per-flight matched cluster points into persistent thermal *core* locations
using spatial density clustering (DBSCAN, haversine). Unlike a border-biased center,
this version computes each waypoint from the DBSCAN *core points only* (when available).
Border points are excluded from the centroid/radius/strength statistics used to define
the waypoint. If a cluster ends up with zero core points (rare), it falls back to all
points in that cluster.

Inputs:
  - Scans: outputs/batch_csv/**/matched_clusters.csv
  - Uses columns: lat, lon, climb_rate_ms (others ignored)
...
Outputs:
  ...
  - Membership CSV: outputs/waypoints/thermal_waypoints_v1_membership.csv (points → core/border + wp_id)
  - Cluster size stats: outputs/waypoints/thermal_waypoints_v1_cluster_sizes.csv

CLI:
  python build_thermals_v1_core.py \
...
def parse_args():
    ap = argparse.ArgumentParser(description="Group matched clusters into persistent thermal CORE locations.")
...
def filter_inputs(df, strength_min, dstart, dend, logfh):
    ...
    # Drop absurd climb spikes/drops (m/s). Adjust if needed.
    pre = len(df2)
...
def run_dbscan_haversine(lat: pd.Series, lon: pd.Series, eps_km: float, min_samples: int) -> Tuple[np.ndarray, np.ndarray]:
    """
    Returns:
      labels: DBSCAN labels for each point (-1 = noise)
      core_mask: boolean mask of shape (n_points,) where True marks a core point
    """
...
def aggregate_clusters(df: pd.DataFrame, labels: np.ndarray, core_mask: np.ndarray,
    ...
    """
    Build waypoints from *core* points only.
    Returns:
    ...
    for lab, grp in df.groupby("__label"):
        # Prefer core points for the core estimate; fallback to all points if no cores (edge case)
        core_grp = grp[grp["__is_core"]]
        use_grp = core_grp if len(core_grp) > 0 else grp

        lat_med, lon_med = robust_median(use_grp["lat"]), robust_median(use_grp["lon"])

        # Compute radius relative to *core* representation (max distance of core points to center)
        if len(use_grp) > 0 and not (math.isnan(lat_med) or math.isnan(lon_med)):
            lat_med_rad, lon_med_rad = np.radians(lat_med), np.radians(lon_med)
            lat_rad, lon_rad = np.radians(use_grp["lat"].values), np.radians(use_grp["lon"].values)
            dists = great_circle_distance_m(lat_med_rad, lon_med_rad, lat_rad, lon_rad)
            radius_m = float(np.max(dists)) if len(dists) else 0.0
            # Optional: a robust radius (80th percentile) can be informative alongside max
            radius_p80_m = float(np.percentile(dists, 80)) if len(dists) else 0.0
        else:
            radius_m = 0.0
            radius_p80_m = 0.0

        # Strength stats from core points (fallback to all points if no cores)
        ...
        # For reference, compute full-cluster stats too
        ...
        rows.append({
            ...
            "encounters": int(len(grp)),            # points in cluster (core + border)
...
def main():
    ...
    def write_empty_outputs():
        pd.DataFrame(columns=[
            ...
            "method","eps_km","min_samples","strength_min",
    ...
    with open(logp, "a") as lf:
        log("=== build_thermals_v1_core start ===", lf)
        ...
        # 3) cluster using the RESOLVED values (ONCE)
        labels, core_mask = run_dbscan_haversine(df.lat, df.lon, eps_km, min_samples)
        out, lab_to_wp = aggregate_clusters(df, labels, core_mask, method, eps_km, min_samples, strength_min)
