...
altitude_gain_v3.py
=================================
Altitude-only detector with **CSV export + DataFrame** output.
...
- Exports results to: outputs/climbs_<yymmddhhmmss>.csv
...
def main():
    ...
    print(f"[alt v3] Parsing IGC: {path}")
    df = parse_igc(path)
    if len(df) == 0:
        print("[alt v3] No rows parsed; aborting."); return
    ...
    try:
        ...
        print(f"[alt v3] Tow detect error: {e}")
        tow = None
    if tow and isinstance(tow, tuple) and len(tow) == 2:
        s,e = tow
        print(f"[alt v3] Tow detected: {s}→{e}. Excluding tow.")
        df = df.iloc[e+1:].reset_index(drop=True)
        df = ensure_time_sec(df)
    else:
        print("[alt v3] Tow not clearly detected; using full trace.")
    print(f"[alt v3] Points after tow trim: {len(df)}")

    # Detect climbs
    segments, vario, alt_s, alt_col, dt = detect_climb_segments(df)
    print(f"[alt v3] Cadence ≈ {dt:.2f} s/fix")
    print(f"[alt v3] Climb segments found: {len(segments)}")

    # Build results DataFrame
    climbs_df = segments_to_dataframe(df, segments)
    total_gain = climbs_df["gain_m"].sum() if not climbs_df.empty else 0.0
    print(f"[alt v3] Total climb across segments: +{total_gain:.0f} m")

    # Ensure outputs folder and write CSV
    out_dir = Path("outputs")
    out_dir.mkdir(parents=True, exist_ok=True)
    csv_path = out_dir / f"climbs_{ts}.csv"
    climbs_df.to_csv(csv_path, index=False)
    print(f"[alt v3] Wrote climbs CSV: {csv_path}   ({len(climbs_df)} rows)")
