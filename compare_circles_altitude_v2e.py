...
"""
compare_circles_altitude_v2e.py
--------------------------------
Purpose
    Compare altitude clusters against circle clusters — with **guardrails integrated**
    into the circle emission BEFORE clustering, so comparisons reflect cleaner inputs.

What's new vs v2d
    • Applies the same TURN-RATE and RADIUS guardrails used by overlay v1j.
    • Keeps both RAW and FILTERED (n≥MIN_CLUSTER_SIZE) cluster views for analysis.
    • Exports both comparisons and the cluster summaries.

Outputs
    - outputs/compare_raw_<ts>.csv
    - outputs/compare_filtered_<ts>.csv
    - outputs/circle_clusters_raw_<ts>.csv
    - outputs/circle_clusters_filtered_<ts>.csv
"""
...
def main():
    ...
    print(f"[compare v2e] Parsing IGC: {igc_file}")
    ...
    if isinstance(tow, tuple) and len(tow) == 2:
        s, e = tow
        print(f"[compare v2e] Tow detected: {s}→{e}. Excluding tow.")
        df = df.iloc[e+1:].reset_index(drop=True)
        df = ensure_time_sec(df)

    # Detect circles and apply guardrails
    circles, dt = detect_circles_multi(df)
    circ_df_raw = circles_to_df(df, circles)
    circ_df = apply_guardrails(circ_df_raw)
    ...
    print(f"[compare] Circles (pre-guardrail): {len(circ_df_raw)} | Circles (kept): {len(circ_df)} | "
