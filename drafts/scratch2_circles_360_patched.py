...
# =========================
# Circles-only (360° refined) detector + plotting
# =========================

def find_circles_360(df: pd.DataFrame,
                     min_circle_heading_deg: float = 340.0,
                     max_circle_heading_deg: float = 380.0,
                     max_len_samples: int = 400):
    """Detect full circles (~360° heading change in one direction)."""
    circles = []
    n = len(df)
    i = 0
    while i < n - 2:
        ...
        while j < n and (j - i) < max_len_samples:
            ...
            # if direction reverses, break
            if direction is not None and (d * direction) < 0:
                break

            cum += d
            if abs(cum) >= min_circle_heading_deg and abs(cum) <= max_circle_heading_deg:
                circles.append((i, j, cum))
                i = j + 1
                break
            j += 1
...
def plot_circles(df: pd.DataFrame, circles: list[tuple[int,int,float]]):
    fig, ax = plt.subplots(figsize=(10, 7))
    # Full track in green
    ax.plot(df["lon"], df["lat"], color="green", lw=1.2, alpha=0.7, label="Track")
    # Circles in blue
    for (s, e, hdg) in circles:
        ax.plot(df.loc[s:e, "lon"], df.loc[s:e, "lat"], color="blue", lw=2.0, alpha=0.9)
        ax.scatter([df.loc[s, "lon"], df.loc[e, "lon"]],
                   [df.loc[s, "lat"], df.loc[e, "lat"]],
                   color="red", s=30, zorder=5)
    ax.set_title("Circles-only detection (360° refined)")
...
def main():
    ...
    print(f"[circles-360] Parsing IGC: {path}")
    df = parse_igc(path)
    df = compute_derived(df)
    tow_seg = detect_tow_segment(df)
    if tow_seg and isinstance(tow_seg, tuple) and len(tow_seg) == 2:
        s, e = tow_seg
        print(f"[circles-360] Tow detected: {s}→{e}. Excluding tow.")
        df = df.iloc[e+1:].reset_index(drop=True)
    else:
        print("[circles-360] Tow not clearly detected; using full trace.")

    circles = find_circles_360(df)
    print(f"[circles-360] Detected {len(circles)} circles")
    for (s, e, hdg) in circles:
        t0, t1 = df.loc[s, 'time'], df.loc[e, 'time']
        print(f"  circle: idx {s}->{e}, samples {e-s}, {t0.time()}→{t1.time()}, heading {hdg:.1f}°")
